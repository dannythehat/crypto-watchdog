...
Build #51 — Content QA & Brand Voice Manager v1 adds the read-only QA layer for CryptoWatchdog tone, evidence-first wording, article formatting, review structure, headings, FAQs, links, media placement notes, affiliate disclosure, and human review needs. Run it with `npm run content:qa-brand` and validate it with `npm run content:qa-brand-validate`. It protects the evidence-first, protection-first, plain-English, sceptical-not-cynical voice and remains report-only: no live edits, publishing, Supabase writes, affiliate URL insertion, approval/apply workflow, AI/API calls, live crawling/fetching, media generation/download/upload, trust rating changes, final legal/policy wording, or scam/fraud accusations.
